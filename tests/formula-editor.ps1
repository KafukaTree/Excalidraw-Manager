...
try {
    ...
    Assert-True ($homeResponse.Body.Contains('id="copy-svg"')) 'SVG is the primary copy action'
    ...
    Assert-True ($editorScript.Body.Contains('groupsRect.bottom - panelRect.top')) 'wrapped quick categories open their popover below every button row'
    ...
    Assert-True ($editorStyles.Body.Contains('grid-template-columns: repeat(auto-fit, minmax(38px, 1fr))')) 'compact formula categories reflow without horizontal scrolling'
    ...
    Assert-True ($overlayScript.Contains("window.open('', '_blank'")) 'board overlay can detach the palette into a resizable desktop popup'
    Assert-True ($overlayScript.Contains('handleSvgInsert(event.data, nextPopup)')) 'detached palette inserts through the validated popup frame without an unsafe relay'
    Assert-True ($overlayScript.Contains('nextPopup.opener = null')) 'detached palette cannot navigate the board through window.opener'
    ...
    Assert-True ($overlayScript.Contains('Math.max(300, Math.min(720')) 'detached formula window can start at a narrow width'
